    ...
    protected void configure(HttpSecurity http) throws Exception {
		...
		*/
        ...
        http.csrf().disable()	// csrf 토큰을 비활성화
			...
			.antMatchers("/", "/member/LoginMember.do", "/js/**","/css/**","/images/**").permitAll() // 해당 경로들은 접근을 허용
		...
		.and()
		    ...
		    .logoutSuccessUrl("/member/LogoutMember.do") // 성공시 리턴 URL
		...
		.and()
            ...
            .expiredUrl("/member/LoginMember.do?error=true&exception=Have been attempted to login from a new place. or session expired")  // 세션이 만료된 경우 이동 할 페이지를 지정
        .and()
	        .and().rememberMe() // 로그인 유지
	        .alwaysRemember(true) // 항상 기억할 것인지 여부
	        .tokenValiditySeconds(604800) // in seconds, 12시간 유지
